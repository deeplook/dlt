...
class NoMigrationPathException(StorageException):
    def __init__(
        ...
        super().__init__(
            f"Could not find migration path for {storage_path} from v {initial_version} to {target_version}, stopped at {migrated_version}"
        )
...
class JobWithUnsupportedWriterException(LoadStorageException):
    def __init__(
        ...
        super().__init__(
            f"Job {wrong_job} for load id {load_id} requires loader file format that is not one of {expected_file_formats}"
        )
...
class UnexpectedSchemaName(SchemaStorageException, ValueError):
    def __init__(self, schema_name: str, storage_path: str, stored_name: str) -> None:
        super().__init__(
            f"A schema file name '{schema_name}' in {storage_path} does not correspond to the name of schema in the file {stored_name}"
        )
