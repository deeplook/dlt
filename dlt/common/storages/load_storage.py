...
class LoadStorage(DataItemStorage, VersionedStorage):
    ...
    NORMALIZED_FOLDER = (
        "normalized"  # folder within the volume where load packages are stored
    )
    ...
    SCHEMA_UPDATES_FILE_NAME = (
        "schema_updates.json"  # updates to the tables in schema created by normalizer
    )
    ...
    ALL_SUPPORTED_FILE_FORMATS: Set[TLoaderFileFormat] = set(
        get_args(TLoaderFileFormat)
    )
    ...
    def _get_data_item_path_template(
        ...
        return self.storage.make_full_path(
            join(load_id, LoadStorage.NEW_JOBS_FOLDER, file_name)
        )
    ...
    def save_temp_schema_updates(
        self, load_id: str, schema_update: TSchemaTables
    ) -> None:
    ...
    def list_packages(self) -> Sequence[str]:
        loads = self.storage.list_folder_dirs(
            LoadStorage.NORMALIZED_FOLDER, to_root=False
        )
    ...
    def list_new_jobs(self, load_id: str) -> Sequence[str]:
        new_jobs = self.storage.list_folder_files(
            self._get_job_folder_path(load_id, LoadStorage.NEW_JOBS_FOLDER)
        )
        ...
        wrong_job = next(
            (
                j
                for j in new_jobs
                if LoadStorage.parse_job_file_name(j).file_format
    ...
    def list_started_jobs(self, load_id: str) -> Sequence[str]:
        return self.storage.list_folder_files(
            self._get_job_folder_path(load_id, LoadStorage.STARTED_JOBS_FOLDER)
        )

    def list_failed_jobs(self, load_id: str) -> Sequence[str]:
        return self.storage.list_folder_files(
            self._get_job_folder_path(load_id, LoadStorage.FAILED_JOBS_FOLDER)
        )
    ...
    def list_completed_failed_jobs(self, load_id: str) -> Sequence[str]:
        return self.storage.list_folder_files(
            self._get_job_folder_completed_path(load_id, LoadStorage.FAILED_JOBS_FOLDER)
        )
        ...
        package_created_at = pendulum.from_timestamp(
            os.path.getmtime(
                self.storage.make_full_path(
                    join(package_path, LoadStorage.PACKAGE_COMPLETED_FILE_NAME)
                )
            )
        )
    ...
    def get_load_package_info(self, load_id: str) -> LoadPackageInfo:
        ...
        if not self.storage.has_folder(package_path):
            ...
            completed_file_path = self.storage.make_full_path(
                join(package_path, LoadStorage.PACKAGE_COMPLETED_FILE_NAME)
            )
            package_created_at = pendulum.from_timestamp(
                os.path.getmtime(completed_file_path)
            )
            package_state = self.storage.load(completed_file_path)
        applied_schema_update_file = join(
            package_path, LoadStorage.APPLIED_SCHEMA_UPDATES_FILE_NAME
        )
    ...
    def commit_schema_update(self, load_id: str, applied_update: TSchemaTables) -> None:
        ...
        processed_schema_update_file = join(
            load_path, LoadStorage.APPLIED_SCHEMA_UPDATES_FILE_NAME
        )
    ...
    def add_new_job(
        ...
        self.storage.atomic_import(
            job_file_path, self._get_job_folder_path(load_id, job_state)
        )
    ...
    def fail_job(
        ...
        if failed_message:
            self.storage.save(
                self._get_job_file_path(
                    load_id, LoadStorage.FAILED_JOBS_FOLDER, file_name + ".exception"
                ),
                failed_message,
            )
        # move to failed jobs
    ...
    def complete_load_package(self, load_id: str, aborted: bool) -> None:
        ...
        self.storage.save(
            join(load_path, LoadStorage.PACKAGE_COMPLETED_FILE_NAME), completed_state
        )
        # move to completed
    ...
    def _get_job_file_path(
        self, load_id: str, folder: TJobState, file_name: str
    ) -> str:
