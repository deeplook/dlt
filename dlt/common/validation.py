...
def validate_dict(
    ...
    def verify_prop(pk: str, pv: Any, t: Any) -> None:
        ...
        if is_literal_type(t):
            ...
            if pv not in a_l:
                raise DictValidationException(
                    f"In {path}: field {pk} value {pv} not in allowed {a_l}",
                    path,
                    pk,
                    pv,
                )
            ...
            if not isinstance(pv, t):
                raise DictValidationException(
                    f"In {path}: field {pk} value {pv} has invalid type {type(pv).__name__} while {t.__name__} is expected",
                    path,
                    pk,
                    pv,
                )
            ...
            if not isinstance(pv, dict):
                raise DictValidationException(
                    f"In {path}: field {pk} value {pv} has invalid type {type(pv).__name__} while dict is expected",
                    path,
                    pk,
                    pv,
                )
            ...
            if not isinstance(pv, list):
                raise DictValidationException(
                    f"In {path}: field {pk} value {pv} has invalid type {type(pv).__name__} while list is expected",
                    path,
                    pk,
                    pv,
                )
            ...
            for d_k, d_v in pv.items():
                if not isinstance(d_k, str):
                    raise DictValidationException(
                        f"In {path}: field {pk} key {d_k} must be a string",
                        path,
                        pk,
                        d_k,
                    )
