...
def init_command_wrapper(
    ...
    try:
        init_command(
            source_name, destination_name, use_generic_template, repo_location, branch
        )
    except Exception as ex:
        click.secho(str(ex), err=True, fg="red")
        fmt.note(
            "Please refer to %s for further assistance" % fmt.bold(DLT_INIT_DOCS_URL)
        )
        return -1
    return 0
...
def telemetry_status_command_wrapper() -> int:
    try:
        ...
        fmt.note(
            ...
            % fmt.bold(DLT_TELEMETRY_DOCS_URL)
        )
        return -1
    return 0
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Non interactive mode. Default choices are automatically made for confirmations and prompts.",
    )
    subparsers = parser.add_subparsers(dest="command")

    init_cmd = subparsers.add_parser(
        "init",
        help="Creates a pipeline project in the current folder by adding existing verified source or creating a new one from template.",
    )
    ...
    init_cmd.add_argument(
        "source",
        nargs="?",
        help="Name of data source for which to create a pipeline. Adds existing verified source or creates a new pipeline template if verified source for your data source is not yet implemented.",
    )
    init_cmd.add_argument(
    ...
    init_cmd.add_argument(
        ...
        default=False,
        action="store_true",
        help="When present uses a generic template with all the dlt loading code present will be used. Otherwise a debug template is used that can be immediately run to get familiar with the dlt sources.",
    )
    ...
    try:
        ...
        deploy_github_cmd.add_argument(
            "--schedule",
            required=True,
            help="A schedule with which to run the pipeline, in cron format. Example: '*/30 * * * *' will run the pipeline every 30 minutes. Remember to enclose the scheduler expression in quotation marks!",
        )
        deploy_github_cmd.add_argument(
            "--run-manually",
            default=True,
        ...
        deploy_cmd = subparsers.add_parser(
            "deploy",
            help='Install additional dependencies with pip install "dlt[cli]" to create deployment packages',
            add_help=False,
        )
    ...
    pipeline_subparsers.add_parser(
        "failed-jobs",
        help="Displays information on all the failed loads in all completed packages, failed jobs and associated error messages",
    )
    pipeline_subparsers.add_parser(
        "sync",
    ...
    pipe_cmd_drop.add_argument(
        "resources",
        nargs="*",
        help="One or more resources to drop. Can be exact resource name(s) or regex pattern(s). Regex patterns must start with re:",
    )
    pipe_cmd_drop.add_argument(
        "--drop-all",
        action="store_true",
    ...
    if Venv.is_virtual_env() and not Venv.is_venv_activated():
        fmt.warning(
            "You are running dlt installed in the global environment, however you have virtual environment activated. The dlt command will not see dependencies from virtual environment. You should uninstall the dlt from global environment and install it in the current virtual environment instead."
        )
