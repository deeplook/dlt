...
class GithubActionDeployment(BaseDeployment):
    ...
    def _generate_workflow(self, *args: Optional[Any]) -> None:
        ...
        if self.schedule_description is None:
            ...
            raise ValueError(f"Setting 'schedule' for '{self.deployment_method}' is required! Use deploy command as 'dlt deploy chess.py {self.deployment_method} --schedule \"*/30 * * * *\"'.")
...
class AirflowDeployment(BaseDeployment):
    ...
    def _echo_instructions(self, *args: Optional[Any]) -> None:
        ...
        fmt.echo("* The airflow %s file was created in %s." % (fmt.bold(AIRFLOW_CLOUDBUILD_YAML), fmt.bold(utils.AIRFLOW_BUILD_FOLDER)))
        fmt.echo(
        ...
        fmt.echo("1. Read this doc and set up the Environment: %s" % (fmt.bold(DLT_AIRFLOW_GCP_DOCS_URL)))
        fmt.echo(
