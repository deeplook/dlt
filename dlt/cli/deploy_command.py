...
def deploy_command(
    ...
    **kwargs: Any,
    ...
    if deployment_method == DeploymentMethods.github_actions.value:
        ...
        raise ValueError(
            f"Deployment method '{deployment_method}' is not supported. Only {', '.join([m.value for m in DeploymentMethods])} are available.'"
        )
...
class GithubActionDeployment(BaseDeployment):
    ...
    def _generate_workflow(self, *args: Optional[Any]) -> None:
        ...
        requirements_txt = generate_pip_freeze(
            requirements_blacklist, REQUIREMENTS_GITHUB_ACTION
        )
    ...
    def _make_modification(self) -> None:
        ...
        self.repo_storage.save(
            os.path.join(
                utils.GITHUB_WORKFLOWS_DIR, self.artifacts["serialized_workflow_name"]
            ),
    ...
    def _echo_instructions(self, *args: Optional[Any]) -> None:
        ...
        fmt.echo(
            "* The dependencies that will be used to run the pipeline are stored in %s. If you change add more dependencies, remember to refresh your deployment by running the same 'deploy' command again."
        ...
        if len(self.secret_envs) == 0 and len(self.envs) == 0:
            ...
            fmt.echo(
                ...
                % (
                    ...
                    fmt.bold(
                        github_origin_to_url(self.origin, "/settings/secrets/actions")
                    ),
                )
            )
            fmt.echo()
        ...
        fmt.echo(
            fmt.bold(
                github_origin_to_url(
                    self.origin,
                    f"/actions/workflows/{self.artifacts['serialized_workflow_name']}",
                )
            )
        )
...
class AirflowDeployment(BaseDeployment):
    ...
    def _generate_workflow(self, *args: Optional[Any]) -> None:
        ...
        req_dep = (
            f"{DLT_PKG_NAME}[{DestinationReference.to_name(self.state['destination'])}]"
        )
    ...
    def _make_modification(self) -> None:
        ...
        dest_cloud_build = os.path.join(
            utils.AIRFLOW_BUILD_FOLDER, AIRFLOW_CLOUDBUILD_YAML
        )
        if not self.repo_storage.has_file(dest_cloud_build):
            self.repo_storage.save(dest_cloud_build, self.artifacts["cloudbuild_file"])
        else:
            fmt.warning(
                f"{AIRFLOW_CLOUDBUILD_YAML} already created. Delete the file and run the deploy command again to re-create."
            )

        dest_dag_script = os.path.join(
            utils.AIRFLOW_DAGS_FOLDER, self.artifacts["dag_script_name"]
        )
    ...
    def _echo_instructions(self, *args: Optional[Any]) -> None:
        ...
        fmt.echo(
            ...
            % (
                ...
                fmt.bold(utils.AIRFLOW_DAGS_FOLDER),
            )
        )
        fmt.echo()
        ...
        fmt.echo(
            "1. Read this doc and set up the Environment: %s"
            ...
            if self.secrets_format == SecretFormats.env.value:
                fmt.echo(
                    ...
                    % (
                        ...
                        fmt.bold(
                            "\n".join(
                                self.env_prov.get_key_name(v.key, *v.sections)
                                for v in self.envs
                            )
                        ),
                    )
                ...
                # build toml
                fmt.echo(
                    f"3. Add the following toml-string in the Google Composer UI as the {SECRETS_TOML_KEY} variable."
                )
                fmt.echo()
        ...
        fmt.note(
            "You may need to add more packages ie. when your source requires additional dependencies"
        )
