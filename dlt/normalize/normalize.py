    ...
    def w_normalize_files(
        ...
        def _get_load_storage(file_format: TLoaderFileFormat) -> LoadStorage:
            ...
            supported_formats = list(
                set(destination_caps.supported_loader_file_formats or [])
                | set(destination_caps.supported_staging_file_formats or [])
            )
            if file_format not in supported_formats:
                if (
                    file_format == "parquet"
                ):  # Give users a helpful error message for parquet
