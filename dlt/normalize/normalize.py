...
class Normalize(Runnable[ProcessPool]):
    ...
    def load_or_create_schema(
        schema_storage: SchemaStorage, schema_name: str
    ) -> Schema:
        try:
    ...
    def w_normalize_files(
        ...
        with Container().injectable_context(destination_caps):
            ...
            try:
                ...
                for extracted_items_file in extracted_items_files:
                    line_no: int = 0
                    root_table_name = NormalizeStorage.parse_normalize_file_name(
                        extracted_items_file
                    ).table_name
                    root_tables.add(root_table_name)
                    logger.debug(
                        f"Processing extracted items in {extracted_items_file} in load_id {load_id} with table name {root_table_name} and schema {schema.name}"
                    )
                    with normalize_storage.storage.open_file(extracted_items_file) as f:
                        ...
                        for line_no, line in enumerate(f):
                            ...
                            logger.debug(
                                f"Processed {line_no} items from file {extracted_items_file}, items {items_count} of total {total_items}"
                            )
                        # if any item found in the file
                        if items_count > 0:
                            populated_root_tables.add(root_table_name)
                            logger.debug(
                                f"Processed total {line_no + 1} lines from file {extracted_items_file}, total items {total_items}"
                            )
        ...
        logger.info(
            f"Processed total {total_items} items in {len(extracted_items_files)} files"
        )

        return schema_updates, total_items, load_storage.closed_files(), row_counts

    @staticmethod
    def _w_normalize_chunk(
        load_storage: LoadStorage,
        schema: Schema,
        load_id: str,
        root_table_name: str,
    ...
    def update_schema(
        self, schema: Schema, schema_updates: List[TSchemaUpdate]
    ) -> None:
    ...
    def map_parallel(
        ...
        for params in param_chunk:
            pending: AsyncResult[TWorkerRV] = self.pool.apply_async(
    ...
    def spool_files(
        ...
        self.load_storage.save_temp_schema_updates(
    ...
    def spool_schema_files(
        self, load_id: str, schema_name: str, files: Sequence[str]
    ) -> str:
    ...
    def run(self, pool: ProcessPool) -> TRunMetrics:
        ...
        return TRunMetrics(
            False, len(self.normalize_storage.list_files_to_normalize_sorted())
        )
