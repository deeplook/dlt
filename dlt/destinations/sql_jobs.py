    ...
    def from_table_chain(
        ...
        try:
            ...
            sql = [
                ...
                for stmt in cls.generate_sql(table_chain, sql_client)
            ]
...
class SqlMergeJob(SqlBaseJob):
    ...
    def gen_key_table_clauses(
        ...
        """
        return [
            f"FROM {root_table_name} as d WHERE EXISTS (SELECT 1 FROM {staging_root_table_name} as s WHERE {' OR '.join([c.format(d='d',s='s') for c in key_clauses])})"
        ]
    ...
    def gen_merge_sql(
        ...
        if len(table_chain) == 1:
            ...
            # delete top table
            sql.append(
                f"DELETE FROM {root_table_name} WHERE {unique_column} IN (SELECT * FROM {delete_temp_table_sql});"
            )
            # delete other tables
            for table in table_chain[1:]:
                ...
                root_key_column = sql_client.capabilities.escape_identifier(
                    root_key_columns[0]
                )
                sql.append(
                    f"DELETE FROM {table_name} WHERE {root_key_column} IN (SELECT * FROM {delete_temp_table_sql});"
                )
