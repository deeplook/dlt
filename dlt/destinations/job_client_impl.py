...
class SqlJobClientBase(JobClientBase, WithStateSync):
    ...
    def update_stored_schema(
        ...
        if schema_info is None:
            logger.info(
                f"Schema with hash {self.schema.stored_version_hash} not found in the storage. upgrading"
            )
            ...
            logger.info(
                f"Schema with hash {self.schema.stored_version_hash} inserted at {schema_info.inserted_at} found in storage, no upgrade required"
            )
    ...
    def _create_optimized_replace_job(
        self, table_chain: Sequence[TTableSchema]
    ) -> NewLoadJob:
    ...
    def create_table_chain_completed_followup_jobs(
        ...
        if write_disposition == "merge":
            ...
            and self.config.replace_strategy == "insert-from-staging"
        ):
            jobs.append(self._create_staging_copy_job(table_chain))
        elif (
            write_disposition == "replace"
            and self.config.replace_strategy == "staging-optimized"
        ):
...
WHERE """
        ...
        if generate_alter:
            ...
            for hint in COLUMN_HINTS:
                if any(c.get(hint, False) is True for c in new_columns):
                    ...
                    if hint == "not_null":
                        logger.warning(
                            f"Column(s) {hint_columns} with NOT NULL are being added to existing table {canonical_name}."
                            " If there's data in the table the operation will fail."
                        )
                    else:
                        logger.warning(
                            f"Column(s) {hint_columns} with hint {hint} are being added to existing table {canonical_name}."
    ...
    def _update_schema_in_storage(self, schema: Schema) -> None:
        ...
        if version_hash != schema.stored_version_hash:
            raise DestinationSchemaTampered(
                schema.name, version_hash, schema.stored_version_hash
            )
    ...
    def _commit_schema_update(self, schema: Schema, schema_str: str) -> None:
        ...
        self.sql_client.execute_sql(
            f"INSERT INTO {name}({self.version_table_schema_columns}) VALUES (%s, %s, %s, %s, %s, %s);",
