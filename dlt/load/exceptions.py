...
class LoadClientJobFailed(DestinationTerminalException):
    def __init__(self, load_id: str, job_id: str, failed_message: str) -> None:
        ...
        super().__init__(
            f"Job for {job_id} failed terminally in load {load_id} with message {failed_message}. The package is aborted and cannot be retried."
        )
...
class LoadClientJobRetry(DestinationTransientException):
    def __init__(
        ...
        super().__init__(
            f"Job for {job_id} had {retry_count} retries which a multiple of {max_retry_count}. Exiting retry loop. You can still rerun the load package to retry this job."
        )
...
class LoadClientUnsupportedFileFormats(DestinationTerminalException):
    def __init__(
        ...
        super().__init__(
            f"Loader does not support writer {file_format} in  file {file_path}. Supported writers: {supported_file_format}"
        )
...
class LoadClientUnsupportedWriteDisposition(DestinationTerminalException):
    def __init__(self, table_name: str, write_disposition: str, file_name: str) -> None:
        ...
        super().__init__(
            f"Loader does not support {write_disposition} in table {table_name} when loading file {file_name}"
        )
