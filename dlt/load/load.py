...
class Load(Runnable[ThreadPool]):
    ...
    def get_staging_destination_client(self, schema: Schema) -> JobClientBase:
        return self.staging_destination.client(
            schema, self.initial_staging_client_config
        )
    ...
    def maybe_with_staging_dataset(
        self, job_client: JobClientBase, table: TTableSchema
    ) -> Iterator[None]:
        """Executes job client methods in context of staging dataset if `table` has `write_disposition` that requires it"""
        if (
            isinstance(job_client, WithStagingDataset)
            and table["write_disposition"] in job_client.get_stage_dispositions()
        ):
    ...
    def w_spool_job(
        ...
        try:
            ...
            with job_client as job_client:
                ...
                logger.info(
                    f"Will load file {file_path} with table name {job_info.table_name}"
                )
                ...
                with self.maybe_with_staging_dataset(job_client, table):
                    job = job_client.start_file_load(
                        table,
                        self.load_storage.storage.make_full_path(file_path),
                        load_id,
                    )
            ...
            job = EmptyLoadJob.from_file_path(
    ...
    def retrieve_jobs(
        ...
        logger.info(
            f"Found {len(started_jobs)} that are already started and should be continued"
        )
        ...
        for file_path in started_jobs:
            try:
                ...
                logger.exception(
                    f"Job retrieval for {file_path} failed, job will be terminated"
                )
                job = EmptyLoadJob.from_file_path(
                    file_path, "failed", pretty_format_exception()
                )
    ...
    def get_new_jobs_info(
        self, load_id: str, schema: Schema, dispositions: List[TWriteDisposition] = None
    ) -> List[ParsedLoadJobFileName]:
        ...
        for job_file in new_job_files:
            if (
                ...
                or self.get_load_table(schema, job_file)["write_disposition"]
                in dispositions
            ):
        ...
        if isinstance(starting_job, FollowupJob):
            ...
            if state == "completed" and not self.is_staging_destination_job(
                ...
                if table_chain := self.get_completed_table_chain(
                    ...
                    schema,
                    top_job_table,
                    starting_job.job_file_info().job_id(),
                ):
    ...
    def complete_jobs(
        ...
        for ii in range(len(jobs)):
            ...
            if state == "running":
                ...
                logger.error(
                    f"Job for {job.job_id()} failed terminally in load {load_id} with message {failed_message}"
                )
            elif state == "retry":
                # try to get exception message from job
                ...
                for followup_job in followup_jobs:
                    ...
                    logger.info(
                        f"Job {job.job_id()} CREATED a new FOLLOWUP JOB {followup_job.new_file_path()} placed in {folder}"
                    )
            ...
            if state in ["failed", "completed"]:
                ...
                if state == "failed":
                    self.collector.update(
                        "Jobs",
                        1,
                        message="WARNING: Some of the jobs failed!",
                        label="Failed",
                    )
        ...
        if not aborted:
            with self.get_destination_client(schema) as job_client:
                ...
                if isinstance(job_client.config, DestinationClientDwhConfiguration):
                    self._processed_load_ids[
                        load_id
                    ] = job_client.config.normalize_dataset_name(schema)
                else:
    ...
    def get_table_chain_tables_for_write_disposition(
        self, load_id: str, schema: Schema, dispositions: List[TWriteDisposition]
    ) -> Set[str]:
        ...
        for job in table_jobs:
            top_job_table = get_top_level_table(
                schema.tables, self.get_load_table(schema, job.job_id())["name"]
            )
            table_chain = get_child_tables(schema.tables, top_job_table["name"])
            for table in table_chain:
                existing_jobs = self.load_storage.list_jobs_for_table(
                    load_id, table["name"]
                )
        ...
        with self.get_destination_client(schema) as job_client:
            ...
            if expected_update is not None:
                ...
                truncate_tables = self.get_table_chain_tables_for_write_disposition(
                    load_id,
                    schema,
                    job_client.get_truncate_destination_table_dispositions(),
                )
                ...
                if isinstance(job_client, WithStagingDataset):
                    if staging_tables := self.get_table_chain_tables_for_write_disposition(
                        load_id, schema, job_client.get_stage_dispositions()
                    ):
                        with job_client.with_staging_dataset():
                            logger.info(
                                f"Client for {job_client.config.destination_name} will start initialize STAGING storage"
                            )
                            job_client.initialize_storage()
                            logger.info(
                                f"Client for {job_client.config.destination_name} will UPDATE STAGING SCHEMA to package schema"
                            )
                            job_client.update_stored_schema(
                                only_tables=staging_tables
                                | {schema.version_table_name},
                                expected_update=expected_update,
                            )
                            logger.info(
                                f"Client for {job_client.config.destination_name} will TRUNCATE STAGING TABLES: {staging_tables}"
                            )
                            job_client.initialize_storage(
                                truncate_tables=staging_tables
                            )
            ...
            if self.staging_destination:
                with self.get_staging_destination_client(schema) as staging_client:
                    jobs_count, jobs = self.retrieve_jobs(
                        job_client, load_id, staging_client
                    )
            else:
    ...
    def run(self, pool: ThreadPool) -> TRunMetrics:
        ...
        logger.info(
            f"Loaded schema name {schema.name} and version {schema.stored_version}"
        )
    ...
    def get_load_info(
        ...
        return LoadInfo(
            ...
            str(self.initial_staging_client_config)
            if self.initial_staging_client_config
            else None,
