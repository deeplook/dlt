...
class Load(Runnable[ThreadPool]):
    ...
    def _init_dataset_and_update_schema(
        ...
        logger.info(
            f"Client for {job_client.config.destination_name} will start initialize storage {staging_text}"
        )
        job_client.initialize_storage()
        logger.info(
            f"Client for {job_client.config.destination_name} will update schema to package schema {staging_text}"
        )
    ...
    def _init_client(
        ...
        tables_with_jobs = set(
            job.table_name for job in self.get_new_jobs_info(load_id)
        )
    ...
    def load_single_package(self, load_id: str, schema: Schema) -> None:
        ...
        with self.get_destination_client(schema) as job_client:
            if (
                ...
                # init staging client
                if self.staging_destination and isinstance(
                    job_client, SupportsStagingDestination
                ):
