...
def test_duckdb_open_conn_default() -> None:
    ...
    try:
        ...
        c = resolve_configuration(
            DuckDbClientConfiguration(dataset_name="test_dataset")
        )
...
def test_duckdb_database_path() -> None:
    ...
    c = resolve_configuration(
        DuckDbClientConfiguration(
            dataset_name="test_dataset", credentials=f"duckdb:///{db_path}"
        )
    )
    ...
    with pytest.raises(duckdb.IOException):
        c = resolve_configuration(
            DuckDbClientConfiguration(
                dataset_name="test_dataset", credentials=TEST_STORAGE_ROOT
            )
        )
        conn = c.credentials.borrow_conn(read_only=False)
...
def test_keeps_initial_db_path() -> None:
    db_path = "_storage/path_test_quack.duckdb"
    p = dlt.pipeline(
        pipeline_name="quack_pipeline", credentials=db_path, destination="duckdb"
    )
    ...
    with p.sql_client() as conn:
        ...
        assert (
            p.get_local_state_val("duckdb_database").lower()
            == os.path.abspath(db_path).lower()
        )
...
def test_duck_database_path_delete() -> None:
    ...
    p = dlt.pipeline(
        pipeline_name="deep_quack_pipeline", credentials=db_path, destination="duckdb"
    )
    p.run([1, 2, 3], table_name="table", dataset_name="dataset")
    # attach the pipeline
...
def test_case_sensitive_database_name() -> None:
    ...
    p = dlt.pipeline(
        pipeline_name="NOT_QUAck", credentials=db_path, destination="duckdb"
    )
    with p.sql_client() as conn:
