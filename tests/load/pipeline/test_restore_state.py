...
from tests.common.utils import (
    ...
    yml_case_path as common_yml_case_path,
)
...
def test_silently_skip_on_invalid_credentials(
    ...
    ] = '{"project_id": "chat-analytics-","client_email": "loader@chat-analytics-317513","private_key": "-----BEGIN PRIVATE KEY-----\\nMIIEuwIBADANBgkqhkiG9w0BAQEFAASCBKUwggShAgEAAoIBAQCNEN0bL39HmD"}'
...
def test_ignore_state_unfinished_load(
    destination_config: DestinationTestConfiguration,
) -> None:
    pipeline_name = "pipe_" + uniq_id()
    dataset_name = "state_test_" + uniq_id()
...
def test_restore_state_parallel_changes(
    destination_config: DestinationTestConfiguration,
) -> None:
    pipeline_name = "pipe_" + uniq_id()
    dataset_name = "state_test_" + uniq_id()
