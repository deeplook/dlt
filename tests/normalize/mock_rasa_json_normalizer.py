...
class DataItemNormalizer(RelationalNormalizer):
    def normalize_data_item(self, source_event: TDataItem, load_id: str, table_name: str) -> TNormalizedRowIterator:
        if self.schema.name == "event":
            ...
            yield from super().normalize_data_item(source_event, load_id, table_name + "_" + source_event["event"])
        else:
