...
def test_extract_select_tables() -> None:
    def expect_tables(resource: DltResource) -> dlt.Schema:
        ...
        clean_test_storage()
        source = DltSource(
            "selectables", "module", dlt.Schema("selectables"), [resource(10)]
        )
        ...
        expect_extracted_file(
            storage, "selectables", "odd_table", json.dumps([1, 3, 5, 7, 9])
        )
        expect_extracted_file(
            storage, "selectables", "even_table", json.dumps([0, 2, 4, 6, 8])
        )
        ...
        source = DltSource(
            "selectables", "module", dlt.Schema("selectables"), [resource]
        )
...
def test_extract_shared_pipe():
    ...
    source = DltSource(
        ...
        [input_r, input_r.with_name("gen_clone")],
    )
    storage = ExtractorStorage(NormalizeStorageConfiguration())
    extract_id = storage.create_extract_id()
    schema_update = extract(extract_id, source, storage)
