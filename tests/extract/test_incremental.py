...
from tests.extract.utils import (
    AssertItems,
    data_to_item_format,
    TItemFormat,
    ALL_ITEM_FORMATS,
...
def test_single_items_last_value_state_is_updated(item_type: TItemFormat) -> None:
    data = [
        {"created_at": 425},
        {"created_at": 426},
    ]
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_batch_items_last_value_state_is_updated(item_type: TItemFormat) -> None:
    ...
    s = p.state["sources"][p.default_schema_name]["resources"]["some_data"][
        "incremental"
    ]["created_at"]
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_last_value_access_in_resource(item_type: TItemFormat) -> None:
    values = []
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_unique_keys_are_deduplicated(item_type: TItemFormat) -> None:
    data1 = [
        {"created_at": 1, "id": "a"},
        {"created_at": 2, "id": "b"},
        {"created_at": 3, "id": "c"},
    ...
    with p.sql_client() as c:
        with c.execute_query(
            "SELECT created_at, id FROM some_data order by created_at, id"
        ) as cur:
            rows = cur.fetchall()
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_unique_rows_by_hash_are_deduplicated(item_type: TItemFormat) -> None:
    data1 = [
        {"created_at": 1, "id": "a"},
        {"created_at": 2, "id": "b"},
        {"created_at": 3, "id": "c"},
...
def test_nested_cursor_path_arrow_fails(item_type: TItemFormat) -> None:
    ...
    assert (
        "Column name data.items.[0].created_at was not found in the arrow table"
        in str(ex)
    )


@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_explicit_initial_value(item_type: TItemFormat) -> None:
    @dlt.resource
    def some_data(created_at=dlt.sources.incremental("created_at")):
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_explicit_incremental_instance(item_type: TItemFormat) -> None:
    ...
    p = dlt.pipeline(pipeline_name=uniq_id())
    p.extract(
        some_data(incremental=dlt.sources.incremental("inserted_at", initial_value=241))
    )
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_optional_incremental_from_config(item_type: TItemFormat) -> None:
    os.environ[
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_optional_incremental_not_passed(item_type: TItemFormat) -> None:
...
def optional_incremental_arg_resource(
    item_type: TItemFormat, incremental: Optional[dlt.sources.incremental[Any]] = None
) -> Any:
    data = [1, 2, 3]
    source_items = data_to_item_format(item_type, data)
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_optional_arg_from_spec_not_passed(item_type: TItemFormat) -> None:
    p = dlt.pipeline(pipeline_name=uniq_id())
...
def some_data_override_config(
    item_type: TItemFormat, created_at: dlt.sources.incremental[str] = dlt.config.value
):
    assert created_at.cursor_path == "created_at"
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_override_initial_value_from_config(item_type: TItemFormat) -> None:
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_override_primary_key_in_pipeline(item_type: TItemFormat) -> None:
    """Primary key hint passed to pipeline is propagated through apply_hints"""
    data = [
        {"created_at": 22, "id": 2, "other_id": 5},
        {"created_at": 22, "id": 2, "other_id": 6},
    ]
    source_items = data_to_item_format(item_type, data)
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_composite_primary_key(item_type: TItemFormat) -> None:
    data = [
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_last_value_func_min(item_type: TItemFormat) -> None:
    data = [
        {"created_at": 10},
        {"created_at": 11},
        {"created_at": 9},
    ...
    @dlt.resource
    def some_data(
        created_at=dlt.sources.incremental("created_at", last_value_func=min)
    ):
        yield from source_items
...
def test_last_value_func_custom() -> None:
    ...
    @dlt.resource
    def some_data(
        created_at=dlt.sources.incremental("created_at", last_value_func=last_value)
    ):
        yield {"created_at": 9}
        yield {"created_at": 10}
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_cursor_datetime_type(item_type: TItemFormat) -> None:
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_descending_order_unique_hashes(item_type: TItemFormat) -> None:
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_unique_keys_json_identifiers(item_type: TItemFormat) -> None:
    ...
    def some_data(last_timestamp=dlt.sources.incremental("ts")):
        data = [
            {"DelTa": i, "ts": pendulum.now().add(days=i).timestamp()}
    ...
    assert (
        p.default_schema.tables["some_data"]["columns"]["del_ta"]["primary_key"] is True
    )
    with p.sql_client() as c:
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_missing_primary_key(item_type: TItemFormat) -> None:
    @dlt.resource(primary_key="DELTA")
    def some_data(last_timestamp=dlt.sources.incremental("ts")):
        data = [
            {"delta": i, "ts": pendulum.now().add(days=i).timestamp()}
...
def test_filter_processed_items(item_type: TItemFormat) -> None:
    ...
    def standalone_some_data(
        item_type: TItemFormat,
        now=None,
        last_timestamp=dlt.sources.incremental("timestamp"),
    ):
        data = [
    ...
    values = list(
        standalone_some_data(item_type, last_timestamp=pendulum.now().timestamp())
    )
    values = data_item_to_list(item_type, values)
    ...
    values = list(
        standalone_some_data(
            ...
            last_timestamp=dlt.sources.incremental(
                "timestamp", pendulum.now().timestamp(), min
            ),
        )
    )
    values = data_item_to_list(item_type, values)
...
def test_start_value_set_to_last_value() -> None:
    ...
    def some_data(step, last_timestamp=dlt.sources.incremental("ts")):
        ...
        if step == -10:
            ...
            assert (
                last_timestamp.start_value == last_timestamp.last_value == expected_last
            )
        ...
        if step == -10:
            ...
            assert (
                last_timestamp.start_value == expected_last != last_timestamp.last_value
            )
...
@pytest.mark.parametrize("item_type", set(ALL_ITEM_FORMATS) - {"json"})
def test_start_value_set_to_last_value_arrow(item_type: TItemFormat) -> None:
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_replace_resets_state(item_type: TItemFormat) -> None:
    p = dlt.pipeline(pipeline_name=uniq_id(), destination="duckdb")
    now = pendulum.now()
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_incremental_as_transform(item_type: TItemFormat) -> None:
    ...
    def some_data():
        last_value: dlt.sources.incremental[
            float
        ] = dlt.sources.incremental.from_existing_state("some_data", "ts")
    ...
    r = some_data().add_step(
        dlt.sources.incremental("ts", initial_value=now, primary_key="delta")
    )
    p = dlt.pipeline(pipeline_name=uniq_id())
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_incremental_explicit_disable_unique_check(item_type: TItemFormat) -> None:
    @dlt.resource(primary_key="delta")
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_apply_hints_incremental(item_type: TItemFormat) -> None:
    p = dlt.pipeline(pipeline_name=uniq_id())
    ...
    r.apply_hints(
        incremental=dlt.sources.incremental("created_at", last_value_func=max)
    )
    p.extract(r)
    assert "incremental" in r.state
    # min value
...
def test_timezone_naive_datetime() -> None:
    ...
    def some_data(
        updated_at: dlt.sources.incremental[
            pendulum.DateTime
        ] = dlt.sources.incremental("updated_at", pendulum_start_dt)
    ):
        data = [
...
def test_chunked_ranges(item_type: TItemFormat) -> None:
    ...
    pipeline = dlt.pipeline(
        pipeline_name="incremental_" + uniq_id(), destination="duckdb"
    )
    ...
    for start, end in chunks:
        pipeline.run(
            endless_sequence(
                item_type,
                updated_at=dlt.sources.incremental(initial_value=start, end_value=end),
            ),
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_end_value_with_batches(item_type: TItemFormat) -> None:
    ...
    pipeline.run(
        batched_sequence(
            updated_at=dlt.sources.incremental(initial_value=1, end_value=10)
        ),
        write_disposition="append",
    )
    ...
    pipeline.run(
        batched_sequence(
            updated_at=dlt.sources.incremental(initial_value=10, end_value=14)
        ),
        write_disposition="append",
    )
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_load_with_end_value_does_not_write_state(item_type: TItemFormat) -> None:
    ...
    pipeline.extract(
        endless_sequence(
            item_type,
            updated_at=dlt.sources.incremental(initial_value=20, end_value=30),
        )
    )
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_end_value_initial_value_errors(item_type: TItemFormat) -> None:
    @dlt.resource
    def some_data(
        updated_at: dlt.sources.incremental[int] = dlt.sources.incremental(
            "updated_at"
    ...
    with pytest.raises(ConfigurationValueError) as ex:
        list(
            some_data(
                updated_at=dlt.sources.incremental(initial_value=42, end_value=22)
            )
        )
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_out_of_range_flags(item_type: TItemFormat) -> None:
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_get_incremental_value_type(item_type: TItemFormat) -> None:
    ...
    assert (
        dlt.sources.incremental("id", initial_value=None).get_incremental_value_type()
        is Any
    )
    ...
    r = test_type_3(
        dlt.sources.incremental[float]("updated_at", allow_external_schedulers=True)
    )
    list(r)
    ...
    r = test_type_4(
        dlt.sources.incremental[str]("updated_at", allow_external_schedulers=True)
    )
    list(r)
    assert r.incremental._incremental.get_incremental_value_type() is str
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_join_env_scheduler(item_type: TItemFormat) -> None:
    @dlt.resource
    def test_type_2(
        updated_at: dlt.sources.incremental[int] = dlt.sources.incremental(
            "updated_at", allow_external_schedulers=True
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_join_env_scheduler_pipeline(item_type: TItemFormat) -> None:
    @dlt.resource
    def test_type_2(
        updated_at: dlt.sources.incremental[int] = dlt.sources.incremental(
            "updated_at", allow_external_schedulers=True
...
@pytest.mark.parametrize("item_type", ALL_ITEM_FORMATS)
def test_allow_external_schedulers(item_type: TItemFormat) -> None:
    @dlt.resource()
    def test_type_2(
        updated_at: dlt.sources.incremental[int] = dlt.sources.incremental(
            "updated_at"
