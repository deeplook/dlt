...
def some_data_from_config(
    call_no: int, created_at: Optional[dlt.sources.incremental[str]] = dlt.secrets.value
):
...
def some_data_override_config(
    created_at: dlt.sources.incremental[str] = dlt.config.value,
):
    assert created_at.cursor_path == "created_at"
...
def test_unique_keys_json_identifiers() -> None:
    ...
    with p.sql_client() as c:
        with c.execute_query(
            "SELECT del_ta FROM some_data WHERE _dlt_load_id = %s",
            load_info.loads_ids[0],
        ) as cur:
            rows = cur.fetchall()
    assert len(rows) == 1
    assert rows[0][0] == 9
...
def standalone_some_data(
    now=None, last_timestamp=dlt.sources.incremental("item.timestamp")
):
    for i in range(-10, 10):
        print(i)
        yield {
            "delta": i,
            "item": {"timestamp": (now or pendulum.now()).add(days=i).timestamp()},
        }
    ...
    values = list(
        standalone_some_data(
            last_timestamp=dlt.sources.incremental(
                "item.timestamp", pendulum.now().timestamp(), min
            )
        )
    )
    assert len(values) == 10
...
def test_incremental_as_transform() -> None:
    ...
    def some_data():
        last_value: dlt.sources.incremental[
            float
        ] = dlt.sources.incremental.from_existing_state("some_data", "item.ts")
    ...
    r = some_data().add_step(
        dlt.sources.incremental("item.ts", initial_value=now, primary_key="delta")
    )
    p = dlt.pipeline(pipeline_name=uniq_id())
...
def test_incremental_explicit_primary_key() -> None:
    @dlt.resource(primary_key="delta")
    def some_data(
        last_timestamp=dlt.sources.incremental("item.ts", primary_key="DELTA")
    ):
        for i in range(-10, 10):
            yield {"delta": i, "item": {"ts": pendulum.now().add(days=i).timestamp()}}
...
def test_apply_hints_incremental() -> None:
    ...
    def some_data_w_default(
        created_at=dlt.sources.incremental("$", last_value_func=min)
    ):
        yield [1, 2, 3]
